...
**Arborescence**\
Algorithmic drawing\
│   ├── jour1_lignes_epaisses : dossier avec le fichier .pde avec le code Processing pour cette figure\
│   ├── jour1_monDessin2 : dossier avec le fichier .pde avec mon code pour l'atelier 1.2\
...
Parmi les figures du libre, j'ai choisi celle qui me semblait être la plus simple : la figure lignes épaisses.\
J'ai copié le code en BASIC pour le traduire en Java ligne par ligne et en essayant de voir s'il y avait des fonctions similaires avec Processing.
...
\
\
L'idée était de faire un triangle composé de trapèze, à partir d'un simple trapèze, avec la librairie Turtle en Python.\
...
C'est une fonction par type de trapèze. Je n'ai pas d'idée de nom pour chaque trapèze, alors je les ai numérotés.\
...
\
\
Faire un grand triangle composé de différents triangles composés de trapèzes auraient été difficile.\
En réfléchissant, j'ai voulu dessiner des montages à partir de ce que j'avais commencé à faire.
J'ai fais une ligne de plusieurs triangles dessinés de suite ainsi qu'une autre rangée. Chaque trapèze est coloré d'une nuance de bleu aléatoire, car la couleur bleu et le blanc est associé au froid, à la glace, à la neige et donc aux montagnes.
...
*2 capture d'écrans de mes magnifiques montagnes : les couleurs changent*
</div>